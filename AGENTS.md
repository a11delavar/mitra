...
- **Overlaps:** JS-based collision calculation is permitted *only* to assign `slot` and `total` overlaps. This data MUST be passed to the `<mitra-event>` instances strictly as CSS Custom Properties (e.g. `--overlap-slot`, `--overlap-total`) rather than inline styles, allowing CSS container queries to cleanly disable the clustering in narrow views (e.g., list views) without fighting JS logic.
...
- **Event Clustering (Interval Graphs):** When resolving parallel events, use strict Interval Graph Coloring logic to segment clusters completely before calculating columns and spans. This prevents disjoint events from falsely grouping via distant shared neighbors.
- **Cross-Day Events:** Do NOT attempt to span a single DOM element across multiple days. Split events crossing midnight at the data/selector layer (e.g., `getEventsForDay`). Pass the *original unmodified time range* to the UI component for accurate text display, but use boolean flags (`continuesNext`, `continuedFromPrevious`) to mathematically clamp the grid rendering between rows `1` and `1441` and to strip border radii visually.
- **Domain-Driven Design (DDD):** Encapsulate domain logic (e.g., `compareTo`, `overlapsWith`, `fallsOnDay`) strictly within Domain Entities (e.g., `CalendarEvent`). Do not leak primitive date math or structural business logic into the UI rendering components.
...
- **Scalable CSS Units:** Use `rem` instead of `px` values for most layouts (padding, margin, font-size, widths) to ensure responsive geometry. `px` should generally only be reserved for absolute rendering rules (like 1px borders, negative outlines, or `@container` query thresholds).
- **CSS Variable Fallbacks:** Never hardcode default fallback values for theme variables (e.g., use `var(--bg)` instead of `var(--bg, #191919)`) since theme variables are guaranteed to exist globally.
