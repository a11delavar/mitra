...
// The CRUD/sync methods below go through `Integration`'s registry, so the engine must be registered
// for 'caldav' the same way the running server registers it (see app/server.ts).
...
describe('CalDAV all-day serialization', () => {
	...
	describe('through the write paths (stubbed client)', () => {
		...
		it('updateEntry writes VALUE=DATE properties carrying the canonical UTC dates, wherever the server runs', async () => {
			...
			// normalize the viewer's local midnights before the write (see features/entries/server/entries.ts).
...
describe('CalDAV credentials on the wire', () => {
	...
	it('never carries a live connection', () => {
		...
		// POPULATE the memo slot first (a stub, not a real client — this asserts about serialization, not
		// about connecting): checking the absence on an account that never opened a connection passes
		// whether or not the `@converter({ out: {} })` is there at all, and a vacuous version of this very
		// test is what let that declaration go missing once — an undeclared property has no converter
		// opting it out, so it rode onto the wire with this account's password inside it.
