...
/**
 * The actual network/CRUD machinery behind {@link Integration.fetchSources}, {@link
 * Integration.syncSourceEntries}, {@link Integration.createEntry}, {@link Integration.updateEntry},
 * {@link Integration.deleteEntry} and {@link Integration.excludeOccurrence} — kept OFF the `Integration`
 * subclass itself and registered separately (see {@link registerEngine}) so that class can stay the
 * frontend's API model without dragging its protocol library (tsdav, ical.js, …) into the browser
 * bundle. A provider whose engine needs none of that (Dev, the local-only fixture) simply doesn't
 * register one and overrides these methods on the class directly instead — {@link Integration}'s own
 * versions only ever run for a type an engine was registered for.
 *
 * Every method receives the `integration` it's acting for as an explicit first argument, since the
 * engine is a SINGLETON serving every account of its type(s) — instance state (credentials, uri,
 * addresses, …) lives on that argument, never on the engine itself. A concrete engine typically narrows
 * it to its own provider class immediately (the registration in server/registerEngines.ts is the thing
...
/** Registers the {@link SyncEngine} that handles a discriminator `type` — see server/registerEngines.ts,
 * the one place this is called, ordered and commented like registerIntegrations.ts's registrations. */
...
export abstract class Integration<TCredentials extends Record<string, any> = any> {
	 ...
	 * infrastructure/model/order.ts). Sorted `asc nulls last`: null = never placed by hand — connection order,
	 ...
	 *
	 * Public, not protected: a registered {@link SyncEngine} is a separate collaborator, not a
	 * subclass, and calls this from its own `syncSourceEntries`.
	 */
	 ...
	 *
	 * Delegates to the registered {@link SyncEngine} — a subclass with no engine (Dev) overrides this
	 * directly, and every override below does the same.
	 */
	 ...
	 * (a scoped series edit shifts them along with the series — see features/recurrence/server/occurrences.ts),
