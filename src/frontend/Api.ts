...
export function updateEvent(entry: Entry) {
	...
	return Api.put<Entry>(`/entries/${entry.id}?tz=${tz()}`, { ...entry, recurrence: entry.recurrence ?? null, reminders: entry.reminders ?? null, participants: entry.participants ?? null, relations: entry.relations ?? null })
}
