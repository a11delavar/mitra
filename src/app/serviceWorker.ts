 ...
 * features/reminders/client/push.ts. It deliberately does nothing else — no caching/offline concerns — so updates to it
...
worker.addEventListener('notificationclick', event => {
	...
	if (event.action === 'snooze') {
		// Hand the payload back to the server; it re-sends after the pause (see features/reminders/server/push.ts).
